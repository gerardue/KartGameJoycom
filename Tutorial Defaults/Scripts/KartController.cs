...
public class KartController : MonoBehaviour
{
    ...
    {
        public KartController.Stats modifiers;
    ...
    public KeyboardInput [] inputsControls;

    // can the kart move?
    public bool canMove = true;
    List<StatPowerup> activePowerupList = new List<StatPowerup>();
    GameObject lastGroundCollided = null;
    KartController.Stats finalStats;

    void Awake()
    {
        Rigidbody = GetComponent<Rigidbody>();
        inputsControls = GetComponents<KeyboardInput>();
    ...
    void GatherInputs()
    {
        ...
        for (int i = 0; i < inputsControls.Length; i++)
        {
            var inputSource = inputsControls[i];
